...
(async () => {
  ...
  try {
    ...
    await writeFile('./config.json', JSON.stringify({
      ...
      level: 999,
      status: 'Battle Royal Lobby - 1/16',
      friendaccept: true,
      inviteaccept: true,
      platform: 'WIN',
  }, null, 2));
  ...
  WSClient.on('connect', function(connection) {
    ...
    connection.on('close', function() {
       ...
       if (latest_payload.payload.sessionId)
       {
          ...
          axios.get(url, {
            headers: {"User-Agent": "++Fortnite+Release-21.20-CL-20788829 Windows/10", "Authorization": "Bearer " + client.Auth.auths.token}
          })
  ...
  const handleCommand = (message, sender) => {
    ...
    if (command === 'skin') {
      ...
      const url = `https://fortnite-public-service-prod11.ol.epicgames.com/fortnite/api/game/v2/matchmakingservice/ticket/player/${client.user.id}?partyPlayerIds=${client.party.members.map(m => m.id).join(",")}&bucketId=13920814::EU:PLAYLIST_PLAYGROUNDV2&player.platform=ANDROID&player.subregions=SAO&player.option.preserveSquad=false&player.option.crossplayOptOut=false&player.option.partyId=${client.party.id}&player.option.splitScreen=false&party.WIN=true&input.KBM=true&player.input=KBM&player.option.microphoneEnabled=true&player.option.linkCode=playlist_playgroundv2&player.option.creative.islandCode=playlist_playgroundv2&player.option.uiLanguage=en`
      axios.get(url, {
